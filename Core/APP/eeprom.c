...
{
    uint8_t temp[16] = {0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF};
...
{
    HAL_I2C_Mem_Write(I2cHandle,AT24BaseAddr|(BlockAddr<<1),(PageAddr<<4)|BitAddr,1,RxData,1,0xFF);
}
...
void at24_PageWrite(I2C_HandleTypeDef* I2cHandle,uint8_t BlockAddr,uint8_t PageAddr,uint8_t BitAddr,uint16_t DataLength,uint8_t* RxData)
{
    ...
    HAL_I2C_Mem_Write(I2cHandle,AT24BaseAddr|(BlockAddr<<1),(PageAddr<<4)|BitAddr,1,RxData,DataLength,0xFF);
}

//任意地址，任意长度按页写
void at24_RandomWrite(I2C_HandleTypeDef* I2cHandle,uint8_t BlockAddr,uint8_t PageAddr,uint8_t BitAddr,uint16_t DataLength,uint8_t* RxData)
{
    if((BitAddr+DataLength) <= 16){//数据未超过一页
        at24_PageWrite(I2cHandle,BlockAddr,PageAddr,BitAddr,DataLength,RxData);
    }else{//数据超过一页
        uint8_t BitAddrOffset = 16 - BitAddr;
        uint8_t PageNum = (DataLength + BitAddr)>>4;
        at24_PageWrite(I2cHandle,BlockAddr,PageAddr,BitAddr,BitAddrOffset,RxData);//第一页
        HAL_Delay(2);//给上面的操作延时，让EEPROM写入完成
        for(uint8_t i = 0;i < PageNum;i++){
            if(i == (PageNum-1)){//最后一页
                at24_PageWrite(I2cHandle,BlockAddr+((i+PageAddr+1)>>4),PageAddr+1+(i%16),0x00,((BitAddr+DataLength)%16),RxData+BitAddrOffset+i*16);//最后一页
                HAL_Delay(2);
            }else{//中间的页
                at24_PageWrite(I2cHandle,BlockAddr+((i+PageAddr+1)>>4),PageAddr+1+(i%16),0x00,16,RxData+BitAddrOffset+i*16);
                HAL_Delay(2);
            }
        }
    }
...
//任意地址，任意长度读
void at24_RandomRead(I2C_HandleTypeDef* I2cHandle,uint8_t BlockAddr,uint8_t PageAddr,uint8_t BitAddr,uint16_t DataLength,uint8_t* RxData)
{
    HAL_I2C_Mem_Read(I2cHandle,(AT24BaseAddr|(BlockAddr<<1))+1,(PageAddr<<4)|BitAddr,1,RxData,DataLength,0xFF);
}
