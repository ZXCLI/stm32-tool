...
KEY KeyEvent[KEY_NUM];
ENCODER EncoderEvent[ENCODER_NUM];

KEY_GPIO KeyEvent_GPIO[KEY_NUM] = {
    {IN4_GPIO_Port,IN4_Pin},//UP
    {IN5_GPIO_Port,IN5_Pin},//DOWN
    {IN3_GPIO_Port,IN3_Pin},//LEFT
    {IN1_GPIO_Port,IN1_Pin},//RIGHT
    {IN2_GPIO_Port,IN2_Pin},//ENTER
};
/*------------------------------------------------------------------------------
  编码器相关
------------------------------------------------------------------------------*/
...
{
    return (((HAL_GPIO_ReadPin(EncoderEvent_GPIO[snack].GPIO_left_part,EncoderEvent_GPIO[snack].GPIO_left_pin))<<1)
            |(HAL_GPIO_ReadPin(EncoderEvent_GPIO[snack].GPIO_right_part,EncoderEvent_GPIO[snack].GPIO_right_pin)))&0x3;
}

void encoder_poll()
{
    for(uint8_t snack = 0;snack < ENCODER_NUM;snack++){

        EncoderEvent[snack].state = ((EncoderEvent[snack].last_state << 2) | encoder_read(snack))&0xf;
...
{
    return HAL_GPIO_ReadPin(KeyEvent_GPIO[snack].GPIO_part,KeyEvent_GPIO[snack].GPIO_pin);
}

void key_poll(void)
{
    for(uint8_t snack = 0;snack < KEY_NUM;snack++){
        KeyEvent[snack].state = ((KeyEvent[snack].last_state << 1) | key_read(snack))&0x03;
        switch(KeyEvent[snack].state) {
            ...
            case 0x00://处于按下状态
                ...
                if(KeyEvent[snack].press_count >= longpress_count){     //直接处理长按事件，如果在抬起时才处理会感觉卡顿
